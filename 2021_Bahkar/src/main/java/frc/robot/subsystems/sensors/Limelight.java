...
public class Limelight {
    ...
    public Pose2d getPose2d() {
        try {
            ...
            Pose2d goalRel = new Pose2d(Units.inchesToMeters(dimensions[0]), Units.inchesToMeters(dimensions[2]),
                    new Rotation2d(Math.toRadians(dimensions[4])));

            double dist = Math.hypot(goalRel.getX(), goalRel.getY());
            double totalAngle = Math.atan2(goalRel.getY(), goalRel.getX()) + goalPose.getRotation().getRadians();

            double botX = Math.cos(totalAngle) * dist + goalPose.getX();
            double botY = Math.sin(totalAngle) * dist + goalPose.getY();

            return new Pose2d(botX, botY,
