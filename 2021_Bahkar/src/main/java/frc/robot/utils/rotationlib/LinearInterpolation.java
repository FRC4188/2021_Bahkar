...
public class LinearInterpolation {
    ...
    public Rotation2d sample(double time) {
        ...
        if (upperIndex == 0 && lowerIndex == 0) {
            ...
            double rotation = ((rotations.get(upperIndex) - rotations.get(lowerIndex))
                    / (times.get(upperIndex) - times.get(lowerIndex))) * time
                    - (((rotations.get(upperIndex) - rotations.get(lowerIndex))
                            / (times.get(upperIndex) - times.get(lowerIndex))) * times.get(lowerIndex)
